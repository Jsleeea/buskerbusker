...
router.post("/", function (request, response) {
  ...
  connection.query(query,function(error, results, fields){ // results : noticeData 정보
    if(error){
      console.log(error);
    }
    else{
      console.log(title, " 내에 답변 삭제 완료");
      connection.query(query_2,function(error_2, results_2, fields_2){ // results_2 : commentData 정보
        if(error_2){
          console.log(error_2);
        }
        else{
